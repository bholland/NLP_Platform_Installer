...
FJSP_Collection_Processor_Model_Data_Ingest = """<?xml version="1.0" encoding="UTF-8"?>
...
<cpeDescription>
    <collectionReader>
        <collectionIterator>
            <descriptor>
                <import location="./FJSP_FolderReader_Model_Data_Ingest_CPE.xml"/>
            </descriptor>
        </collectionIterator>
    </collectionReader>
    <casProcessors casPoolSize="$CAS_POOL_SIZE" processingUnitThreadCount="$THREADS">
        <casProcessor deployment="integrated" name="Do Nothing">
            <descriptor>
...
def generate_FJSP_Collection_Processor_Model_Data_Ingest(output_file, cas_pool_size, threads):
    """output_file: a pathlib object"""
    s = Template(FJSP_Collection_Processor_Model_Data_Ingest)
